...
# the time-window and batch version of DCRNN    
class DCRNN2(torch.nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, batch_size: int, timewin_size: int, K: int, bias: bool = True):
        super(DCRNN2, self).__init__()

        self.in_channels = in_channels
        self.out_channels = out_channels
        self.batch_size = batch_size # not used. batch_size is inferred from X in forward()
        self.timewin_size = timewin_size
        self.K = K
        self.bias = bias
    ...
    def _set_hidden_state(self, X, H):
        if H is None:
            # H = torch.zeros(X.shape[0], self.out_channels).to(X.device)
            H = torch.zeros(X.shape[0], X.shape[1], self.out_channels).to(X.device) #(batch, node_num, rnn_units)
        return H
    ...
    def forward(
        ...
        # TODO: loop over the temporal dimension to go through the RNN steps
        for seq in range(self.timewin_size):
            X_seq = X[:, :, :, seq] # (batch, node_num, feature_num)
            H = self._set_hidden_state(X_seq, H)
            Z = self._calculate_update_gate(X_seq, edge_index, edge_weight, H)
            R = self._calculate_reset_gate(X_seq, edge_index, edge_weight, H)
            H_tilde = self._calculate_candidate_state(X_seq, edge_index, edge_weight, H, R)
            H = self._calculate_hidden_state(Z, H, H_tilde)
        # H = self._set_hidden_state(X, H)
        # Z = self._calculate_update_gate(X, edge_index, edge_weight, H)
        # R = self._calculate_reset_gate(X, edge_index, edge_weight, H)
